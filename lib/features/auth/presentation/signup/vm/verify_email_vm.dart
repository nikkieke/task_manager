...
final sendTokenProvider = StateNotifierProvider.autoDispose<SendTokenNotifier,
    AsyncValue<String>>(SendTokenNotifier.new);
...
final verifyEmailTokenProvider = StateNotifierProvider.autoDispose<VerifyEmailNotifier,
    AsyncValue<String>>(VerifyEmailNotifier.new);
...
class VerifyEmailNotifier extends StateNotifier<AsyncValue<String>>{
  ...
  Future<void> sendVerifyEmailToken(String token)async{
    state = const AsyncValue.loading();
    final result = await ref.read(authRepoProvider).confirmEmailVerificationToken(token);
    if(result.isRight) {
      state = AsyncValue.data(result.right.data);
